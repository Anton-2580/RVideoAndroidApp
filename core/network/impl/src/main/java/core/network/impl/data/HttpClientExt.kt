...
suspend inline fun <T> safeCall(
    ...
    val response = try {
        ...
        return Result.Error(NetworkStatus.Error(
            status = NetworkStatus.NoInternet,
            message = e.message ?: NetworkStatus.NoInternet.message,
        ))
    } catch (e: SerializationException) {
        return Result.Error(NetworkStatus.Error(
            status = NetworkStatus.Serialization,
            message = e.message ?: NetworkStatus.Serialization.message,
        ))
    } catch (e: Exception) {
        return Result.Error(NetworkStatus.Error(
            status = NetworkStatus.Unknown,
            message = e.message ?: NetworkStatus.Unknown.message,
        ))
    }
...
suspend inline fun <T: Any> responseToResult(
    ...
    for (networkStatus in NetworkStatus.valuesSealed.getInstances()) {
